...
/**
 * Revisione + applicazione di una bozza di onboarding GIÀ generata --
 * estratto da `PannelloOnboardingAI.tsx` il 15/09/2026 per essere riusato
 * anche da `OnboardingWizard.tsx` (onboarding a domande guidate, richiesta
 * esplicita di Gabriel): la generazione della bozza può arrivare da un testo
 * libero o da risposte guidate, ma la revisione riga per riga e la regola
 * "l'AI non scrive mai senza conferma esplicita" restano identiche in
 * entrambi i casi -- un solo posto dove vive questa logica, non due copie da
...
export function RevisioneBozzaOnboarding({
  ...
  const [stato, setStato] = useState<Stato>({ fase: "revisione", revisione: costruisciRevisione(bozzaIniziale) });
